...
class DQN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.fc4(x))
        return self.head(x)
class DQN_small_maze(nn.Module):
    def __init__(self, in_channels=1, n_actions=4):
        """
        Initialize Deep Q Network
        ...
        Args:
            ...
            n_actions (int): number of outputs
        """
        super(DQN_small_maze, self).__init__()
        # Adjusted convolutional layers for a 16x16 input
        self.conv1 = nn.Conv2d(in_channels, 32, kernel_size=3, stride=2)  # Output: 7x7
        self.conv2 = nn.Conv2d(32, 64, kernel_size=3, stride=2)           # Output: 3x3
        self.conv3 = nn.Conv2d(64, 64, kernel_size=3, stride=1)           # Output: 1x1
        
        # Calculate the output size of the conv layers
        conv_out_size = self._get_conv_out((in_channels, 16, 16))
        print(conv_out_size)
        self.fc4 = nn.Linear(1600, 64)
        self.head = nn.Linear(64, n_actions)
